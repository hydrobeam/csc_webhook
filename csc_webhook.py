...
def gen_message(embed: Embed) -> int:
    """Sends a message to the webhook channel and returns its message_id.

    Will be deleted by delete_message then regenerated when needed to avoid
    `edited` tags on messages.
    """
...
def delete_message():
    ...
    Deletes the message associated with office_status's message_id.

    Typically instantly regenerated by gen_message.
    """
    requests.delete(
...
def main_func(status: dict[str, int] | None = None):
    ...
    if maybe is not None:
        delete_message()
        office_status.message_id = gen_message(maybe)
...
class OfficeStatus:
    ...
    ----------
    message_id: int
        The id of the message that the class currently points to.
